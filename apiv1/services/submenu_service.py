...
class SubmenuService:
    def __init__(
        self,
        operations: SubmenuOperations = Depends(),
    ...
    async def get_submenus(self, menu_id: int) -> list[Submenu | None]:
        return await self.operations.get_many(menu_id)

    async def create_submenu(
        self, menu_id: int, submenu_data: SubmenuCreate
    ) -> SubmenuResponse:
        ...
        parsed_submenu = SubmenuResponse.from_orm(submenu)
        await self.submenu_cache.set(int(submenu.id), parsed_submenu)
        await self.menu_cache.set_submenus_count(menu_id, "add")
        return parsed_submenu
    ...
    async def update_submenu(
        ...
        parsed_submenu = SubmenuResponse.from_orm(submenu)
        await self.submenu_cache.set(submenu_id, parsed_submenu)
        return parsed_submenu
